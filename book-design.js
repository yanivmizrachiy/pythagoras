...
function ensurePreviewColor(root){const app=root.closest?.('#app')||root.querySelector?.('#app');if(app){app.classList.remove('book-theme-original','book-theme-mono');app.classList.add('book-theme-color')}}
function apply(scope=document){if(scope.matches?.('.fallback'))normalize(scope);scope.querySelectorAll?.('.fallback').forEach(normalize);ensurePreviewColor(scope);window.__PYTHAGORAS_DESIGN_AUDIT__=audit(document);window.__PYTHAGORAS_COLOR_COVERAGE__=coverage(document);return window.__PYTHAGORAS_DESIGN_AUDIT__}
