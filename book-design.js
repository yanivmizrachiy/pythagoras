...
function addByLabels(root){root.querySelectorAll('h1,h2,h3,h4,h5,h6,strong,b,p,span,label').forEach(el=>{const t=(el.textContent||'').replace(/\s+/g,' ').trim();if(!t||t.length>80)return;const c=usefulContainer(el);if(/^דוגמ(?:ה|א)|דוגמה פתורה/.test(t))c.classList.add('ped-example');if(/^(רמז|שים לב|שימו לב|זכרו|זכור)/.test(t))c.classList.add('ped-hint');if(/^(פתרון|דרך פתרון|שלבי פתרון)/.test(t))c.classList.add('ped-solution');if(/^(תשובה|תשובת התלמיד)/.test(t))c.classList.add('ped-answer');if(/^(תרגיל|שאלה)\s*\d*/.test(t))c.classList.add('ped-question');if(/^(חשוב|שימו לב)/.test(t))c.classList.add('ped-action')})}
function addStructural(root){root.querySelectorAll('h1').forEach(e=>e.classList.add('ped-page-title'));root.querySelectorAll('h2').forEach(e=>e.classList.add('ped-section-title'));root.querySelectorAll('h3,h4,h5,h6').forEach(e=>e.classList.add('ped-subtitle'));root.querySelectorAll('svg').forEach(svg=>usefulContainer(svg).classList.add('ped-diagram'));root.querySelectorAll('math,.math,math-field,[data-math],[class*="formula" i],[class*="equation" i]').forEach(e=>usefulContainer(e).classList.add('ped-formula'));root.querySelectorAll('input,textarea,[contenteditable="true"],[class*="answer" i],[class*="blank" i]').forEach(e=>usefulContainer(e).classList.add('ped-answer'))}
